...
//replace these with actual string indices
int TV1_idx = 0;
int TV2_idx = 1;
int V2V_idx = 2;
...
void loop() {
  ...
  while (roundNumber == 1) {
    hueToMatch = random8(); //is it just TV or is the whole tree?
    float delay_ms = 10.0;
    for (int i = 50; i < MAX_BRIGHTNESS; i++ ) {
      for (int j = 0; j < NUM_LEDS; j++) {
        for (int string = 0; string < NUM_STRINGS; string++) {
          leds[string][j] = CHSV(hueToMatch, 170, i);
        }
      }
      delay(delay_ms);
      process_synapses();
  ...
  bool round1Complete = 0;
  //when player 1 chooses correct color, signal is received 
  if (round1Complete) {
    //flicker?
    //pulse a lot?
    roundNumber ++;
