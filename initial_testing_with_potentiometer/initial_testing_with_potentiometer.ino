...
void loop() {
  ...
  // get the hue value
  potentiometerHue = getPotentiometerHue();
  Serial.println(potentiometerHue);

  curr = millis();
  //Serial.println(curr - prev);
  prev = curr;
  for (int i = 0; i < NUM_STRINGS; i++) {
   HUE[i] = potentiometerHue; 
  }
  for (int j = 0; j < NUM_LEDS; j++) {
      for (int string = 0; string < NUM_STRINGS; string++) {
        leds[string][j] = CHSV(potentiometerHue, 170, 255);
      }
  }
  FastLED.show();
  for (int j = 0; j < NUM_LEDS; j++) {
...
void process_synapses() {
  if (is_firing) {
    for (int string = 0; string < NUM_STRINGS; string++) {
      leds[string][firing_idx] = CHSV(potentiometerHue, 255, 255);
    }
